...
echo -e "${YELLOW}Do you have a Gemini API key? (Enter it now or press Enter to skip)${NC}"
read -p "Gemini API Key: " GEMINI_KEY

if [ -z "$GEMINI_KEY" ]; then
    GEMINI_KEY="\$(GEMINI_API_KEY)"
    echo -e "${YELLOW}⚠️  No API key provided. You'll need to update it later.${NC}"
fi
